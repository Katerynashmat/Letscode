...
public class UserService implements UserDetailsService {
    ...
    public UserService(UserRepository userRepository, ItemEntityRepository itemEntityRepository){
    ...
    private void sendMessage(User user) {
        if(!StringUtils.isEmpty(user.getEmail())){
            String message = String.format(
                    "Hello, %s! \n"+
                            "Welcome to Some Java Project. Please, visit next link: http://localhost:8080/activate/%s",
                    user.getUsername(),
    ...
    public void deleteUser(User user){
       itemEntityRepository.findByOwner(user).forEach((x)->{
           itemEntityRepository.delete(x);
       });
